    ...
    for batch_idx in range(predictions.size(0)):  # Loop through the batch dimension (B)
        ...
        for class_idx in range(num_classes):  # Loop through the classes
            ...
            if roi is not None:
                ...
                cardinality = (
                        torch.sum(curr_predictions * curr_roi, dim=(0, 1)) +
                        torch.sum(curr_target * curr_roi, dim=(0, 1))
                )
            else:
                # No ROI, just compute intersection and cardinality
                intersection = torch.sum(curr_predictions * curr_target, dim=(0, 1))
                cardinality = (
                        torch.sum(curr_predictions, dim=(0, 1)) +
                        torch.sum(curr_target, dim=(0, 1))
                )
...
def get_sample_wise_dice(outputs: dict[str, Any], batch: dict[str, Any], roi: torch.Tensor, num_classes: int) -> List[dict[int, float]]:
    dices = []
...
class LogImagesCallback(pl.Callback):
    def __init__(self, color_map: dict[int, str], num_classes: int,
                 plot_dice: bool = True,
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0) -> None:
        ...
        if trainer.current_epoch % self._plot_every_n_epochs == 0:
            ...
            if scanner_name not in self._already_seen_scanner:
                self._plot_and_log(val_images_numpy, val_predictions_numpy, val_targets_numpy, pl_module,
                                trainer.global_step, batch_idx, scanner_name, dices)
    ...
    def _plot_and_log(self, images_numpy, predictions_numpy, targets_numpy, pl_module, step, batch_idx, scanner_name,
                      dices) -> None:
