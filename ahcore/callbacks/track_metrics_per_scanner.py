...
class TrackMetricsPerScanner(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        if self._aperio_batch_count > 0:
            averaged_aperio_metrics = {
                f"Aperio/{key}": value / self._aperio_batch_count for key, value in self._aperio_metrics.items()
            }
        ...
        if self._p1000_batch_count > 0:
            averaged_p1000_metrics = {
                f"P1000/{key}": value / self._p1000_batch_count for key, value in self._p1000_metrics.items()
            }
